...
class WatermarkEvaluator:
    ...
    def apply_watermark(
        ...
        if watermark_type == "random_50":
            ...
            adj_adv_indices = [
                i for i, pos in enumerate(pos_tags) 
                if pos in ["ADJ", "ADV"]
            ]
            mask_indices = np.random.choice(
                adj_adv_indices,
                size=int(len(adj_adv_indices) * 0.9),
                replace=False
            )
            for idx in mask_indices:
                modified_tokens[idx] = int(modified_tokens[idx] * reduction_factor)
            ...
            mask_indices = np.random.choice(
                feminine_indices,
                size=int(len(feminine_indices) * 0.9),
                replace=False
            )
            for idx in mask_indices:
                modified_tokens[idx] = int(modified_tokens[idx] * reduction_factor)
            ...
            mask_indices = np.random.choice(
                verb_comp_indices,
                size=int(len(verb_comp_indices) * 0.9),
                replace=False
            )
            for idx in mask_indices:
                modified_tokens[idx] = int(modified_tokens[idx] * reduction_factor)
