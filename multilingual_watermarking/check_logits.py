...
def print_sequence_analysis(text: str, total_log_prob: float, token_probs: list[dict]):
    ...
    for info in token_probs:
        print(
            f"{info['position']:<10} {info['token']:<15} {info['token_text']:<15} {info['probability']:.4e} {info['log_probability']:.4f}"
        )
...
total_log_prob, token_probs = calculate_sequence_probability(
